...
class Encoder(tfkl.Layer):
    def __init__(self, name = "encoder", **kwargs):
        ...
        self.max_sequence_length = kwargs.get("max_sequence_length", 128)

        self.convs = tfk.Sequential([ConvNorm(name = f"enc_conv_{i}", filters = 512,
                                              kernel_size = 5, strides = 1, dilation_rate = 1, activation = "relu")
                                     for i in range(3)])
    ...
    def call(self, mel_spec, speak_emb):
        masking = tfkl.Masking(mask_value = -1.0, input_shape = (self.max_sequence_length, self.mel_feature_dim))
        batch_size = speak_emb.shape[0]

        mel_spec = tfk.preprocessing.sequence.pad_sequences(mel_spec, maxlen = self.max_sequence_length,
                                                            padding = "post",
                                                            value = -1.0, truncating = "post")
        mel_spec = masking(mel_spec)

        mel_spec = tf.transpose(mel_spec, [0, 2, 1])
        speak_emb = tf.broadcast_to(tf.expand_dims(speak_emb, axis = -1),
                                    [batch_size, self.dim_emb, self.max_sequence_length])
