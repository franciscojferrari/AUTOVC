...
class Encoder(tfkl.Layer):
    ...
    def build(self, input_shape):
        ...
        self.lstms = [
            tfkl.Bidirectional(
                tfkl.LSTM(name=f"enc_lstm_{i}", units=self.dim_neck, return_sequences=True))
            for i in range(2)]
...
class UpSampling(tfkl.Layer):
    ...
    def build(self, input_shape):
        self.up_sampling = tf.keras.layers.UpSampling1D(
            name="upsampling", size=self.time_dim // self.dim_neck)
...
class AutoVC(tfk.Model):
    def __init__(self, name="AutoVC", *args, **kwargs):
        ...
        self.input1 = tfkl.InputLayer(
            input_shape=[self.time_dim, self.mel_dim])
        self.input2 = tfkl.InputLayer(input_shape=[self.dim_neck, ])
        self.input3 = tfkl.InputLayer(input_shape=[self.dim_neck, ])
    ...
    def call(self, inputs):
        ...
        mel_spec = self.input1(mel_spec)
        speaker_emb = self.input2(speaker_emb)
        trg_speaker_emb = self.input3(trg_speaker_emb)
